#!/usr/bin/env python3
"""
e15_permissions.py: Implementation of Rule-Based Permission Governance. Episode 15.
...
tool calls were executed immediately. e15 wraps every tool execution with 
...
from core import (
    ...
    ASYNC_DISPATCH,      # Async tool implementations (awaitable-safe)
...
def _guarded(tool_name: str, inp: Dict[str, Any]):
    """
    ...
    return ASYNC_DISPATCH[tool_name](inp)


# === Guarded Dispatch Map ===

# We rebuild the dispatch map, wrapping every single entry with the _guarded function.
# The underlying tool implementations are async-capable; the core dispatch will await them when needed.
PERM_DISPATCH: Dict[str, Callable[[Dict[str, Any]], Any]] = {
    name: (lambda tool_name: (lambda inp: _guarded(tool_name, inp)))(name)
    for name in ASYNC_DISPATCH.keys()
}
...
def main() -> None:
    ...
    print("\033[90mEpisode 15: permission governance | rules from config/permissions.yaml\033[0m\n")
    ...
    while True:
        try:
            ...
            query: str = input("\033[36me15 >> \033[0m").strip()

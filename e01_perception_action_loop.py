#!/usr/bin/env python3
"""
perception_action_loop.py — Episode 01: The Perception-Action Loop

Motto: "One loop + bash = an agent"

The simplest possible agent: perceive (call the LLM) → act (run bash) →
repeat until the model has a final answer. No streaming, no extra tools,
just the raw loop made visible.

Shared primitives (client, tools, dispatch_tools) live in core.py and are
imported here so this file can stay focused on the loop logic itself.
"""

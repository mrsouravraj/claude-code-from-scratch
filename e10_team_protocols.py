#!/usr/bin/env python3
"""
e10_team_protocols.py: Implementation of an FSM-Governed Agent Communication Protocol. Episode 10.
...
def _ensure_mailbox_dir() -> None:
    """Create the mailbox directory when needed (avoid import-time side effects)."""
...
class ProtocolAgent:
    ...
    def handle(self, message: str) -> str:
        ...
        while True:
            ...
            results = dispatch_tools(response.content, ASYNC_DISPATCH)
...
def main() -> None:
    """
    Initializes the terminal interaction for the Episode 10 'FSM Protocol' agent.
    """
    # UI Header in Gray
    # Lists the possible states defined in the AgentState Enum
    states_list = [s.value for s in AgentState]
    print(f"\033[90mEpisode 10: FSM protocol | states: {states_list}\033[0m\n")
    ...
    while True:
        try:
            # User Prompt in Cyan
            query: str = input("\033[36me10 >> \033[0m").strip()

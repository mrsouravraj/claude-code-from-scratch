#!/usr/bin/env python3
"""
task_system.py: Implementation of a Persistent, Dependency-Aware Task Graph. Episode 07.
...
simple list (Episode 03) to a robust, file-based task management system. It allows 
...
Key Architectural Concepts:
    ...
    3. State Persistence: The entire graph is serialized to `.agent_tasks.json`, 
       ...
       off to other agents (Episode 09+).
...
TASK_DISPATCH: Dict[str, Any] = {
    **ASYNC_DISPATCH, # Inherit base tools (async implementations)
...
def main() -> None:
    """
    Initializes the terminal interaction for the Episode 07 'Task System' agent.
    """
    # UI Header in Gray
    print(f"\033[90mEpisode 07: file-based task graph | tasks → {TASKS_FILE}\033[0m\n")
    ...
    while True:
        try:
            # User Prompt in Cyan
            query: str = input("\033[36me07 >> \033[0m").strip()

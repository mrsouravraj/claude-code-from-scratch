...
# Directories to always exclude from grep/glob searches
_EXCLUDE_DIRS: List[str] = [
    ".venv", "venv", ".git", "__pycache__", "node_modules", ".mypy_cache",
]
...
def run_grep(pattern: str, path: str = ".", recursive: bool = True) -> str:
    ...
    Automatically excludes virtual environments, caches, and VCS directories.

    Args:
        pattern (str): The regex pattern to look for.
        path (str): Search root directory (default: current directory).
    ...
    try:
        ...
        # Build --exclude-dir flags so .venv, .git, etc. are never scanned
        exclude_flags = [f"--exclude-dir={d}" for d in _EXCLUDE_DIRS]
        result = subprocess.run(
            ["grep", "-n", *flags, *exclude_flags, pattern, path],
            capture_output=True, text=True, timeout=30
        )
        return ((result.stdout + result.stderr).strip() or "(no matches)")[:10000]
    except FileNotFoundError:
        # Fallback for Windows (no grep installed)
        try:
